...
1. `[스크린샷1_홈대시보드.png]` — 홈 대시보드: Recovery Score 게이지 + 코칭 메시지 카드
2. `[스크린샷2_회복게이지_메세지.png]` — "오늘의 분석" 입력 화면과 회복도 분석 결과(점수 카드·상세표)
3. `[스크린샷3_시각화리포트.png]` — "시각화 리포트" 탭의 활동 캘린더 및 주간/월간 요약 그래프
4. (선택) `[스크린샷3_시각화리포트.png]` — 관리자 "DB 현황"/"관리자 데모" 탭
